...
$(document).ready(function(){
    ...
    if (isError) {
        $("#inmate").html("<p>There was an error fetching data from the Beaufort County Detention Center. Either the URL is malformed or the data is not available. Click or tap the button below to try again.</p>");
    };
    ...
    if (detaineeDate != null) {
        ...
        if (qStart != null) {
            $("#dateRange").html("Bookings from "+toLocaleFromIso(startDate)+" to "+ toLocaleFromIso(endDate));
        }
    ...
    $("#dateSubmitBtn").on("click", function(){
        ...
        var termsAtDateSubmit = $("#filterInput").val().trim().toLowerCase();
        ...
        // update date range
        $("#dateRange").html("Bookings from " + toLocaleFromIso(startVal) + " to " +toLocaleFromIso(endVal));
    ...
    $(".dateShortcut").on("click", function(){
        ...
        var message = (timeBack==="1") ? "Today's bookings" : "Bookings for the last "+ timeBack + " days";

        $("#dateRange").html(message);

        // check for filter text
        var terms = $("#filterInput").val().trim().toLowerCase();
...
function getData(start,end,qStart,qEnd,terms) {
    ...
    $("#inmate, #inmates").html('<div id="loadSpinner"><i class="fa fa-spinner fa-pulse fa-5x fa-fw"></i><span class="sr-only">Loading...</span></div>');
    ...
    $("#dateSortIcon").removeClass().addClass("fa fa-arrow-down");;
    ...
    $.get(ajaxSrc + "/" + start + "/" + end, function(response){
        if (response.success) {
            ...
            $("#bookingPanel, #inmates").html("<p>There was an error fetching data from the Beaufort County Detention Center. Please try again later.</p>");
        };
    });
}
...
function displayInmates(data,start,end,terms) {
    ...
    for (var i=0;i<data.length;i++){ 
        ...
        // build content block
        var inmateBlock = "<div data-booking-number=\"" + detainee.booknum + "\" data-booktime = \"" + detainee.booktime + "\" class=\"detaineeIndex col-lg-2 col-md-2 col-sm-4 col-xs-6\">";  
        
        // start mugshot/details row
        inmateBlock += "<div class=\"row\"><div class=\"col-lg-12\">";

        // photo; if image error, fall back to "no photo" option
        inmateBlock+= '<img onerror="this.src=\'http://media.islandpacket.com/static/news/crime/mugshots/noPhoto.jpg\'" class="bcmugshot" src="' + detainee.photo + '" title="' + detainee.first + ' ' + detainee.middle + ' ' + detainee.last+ '" alt="' + detainee.first + ' ' + detainee.middle + ' ' + detainee.last + '"/><div class="caption"><h2 class="name">' + detainee.first + ' ' + detainee.last + '</h2><p class="booktime"><strong>Booked:</strong> ' + detainee.booktime + '</p></div></div></div>';
    ...
    $(".detaineeIndex").on("click",function(){
        ...
        var bookdate = $(this).attr("data-booktime").split(" ")[1].split("/");
        ...
        var bookingData = {
            ...
            date: bookdate,
            terms: encodeURI($("#filterInput").val().trim().toLowerCase()),
            start: start,
            end: end
        }
	...
	$("#filterInput").off().on("keyup", function(event){
        ...
        if (event.which === 32 || event.which === 13) {
            ...
            // reveal spinner to cue user that filtering is occuring
            $("#filterSpinner").show("fast", function(){ runFilter(value) });
            return;
        }
    ...
    $("#clearFilter").off().on("click",function(){
    ...
    $("#sortAlpha").off().on("click", function(){
        // get current filter terms
        var currentTerms = $("#filterInput").val().trim().toLowerCase();
        ...
        if ($(this).attr("data-sort") === "desc") {
            ...
            $("#alphaSortIcon").removeClass().addClass("fa fa-arrow-down");
            ...
            $("#alphaSortIcon").removeClass().addClass("fa fa-arrow-up");
    ...
    $("#sortDate").off().on("click", function(){
        // get current filter terms
        var currentTerms = $("#filterInput").val().trim().toLowerCase();
        ...
        if ($(this).attr("data-sort") === "asc") {
            $(this).attr("data-sort","desc");
            $("#dateSortIcon").removeClass().addClass("fa fa-arrow-up");
...
function displayInmate(data,start,end,terms) {
    for (var i=0;i<data.length;i++){ 
        ...
        if (inmate.booknum === bn) {
            ...
            // start photo column
            var inmateBlock = '<div class="row"><div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">';

            inmateBlock += '<img onerror="this.src=\'http://media.islandpacket.com/static/news/crime/mugshots/noPhoto.jpg\'" class="bcmugshot" src="' + inmate.photo + '" title="' + inmate.name + '" alt="' + inmate.name + '" /></div>';
            
            // set up detail column, including Return button
            inmateBlock += '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12"><h4>Arrest information <button class="btn btn-primary pull-right" id="returnBtn">Return to Local Arrests</button></h4>';
            
            // start details table
            inmateBlock += '<table class="details table table-hover table-striped"><tr><th>Booking date / time</th><td>' + inmate.booktime + '</td></tr><tr><th>Release date</th><td>' + inmate.reldate + '</td></tr><tr><th>Height / weight</th><td>' + inmate.height + ', ' + inmate.weight + 'lbs.</td></tr><tr><th>Date of birth</th><td>' + inmate.dob + ' (age ' + inmate.age + ')</td></tr><tr><th>City/ZIP</th><td>' + inmate.city + '</td></tr><tr><th>Race / gender</th><td>' + inmate.race + ', ' + inmate.sex + '</td></tr></table>';
            
            // add div at end to hook in details
            inmateBlock += '<div class="row"><div id="arrestDetails" class="col-lg-12"></div></div>';
            ...
            if (inmate.arrestinfo[0].present) {
                ...
                for (var arrestIndex = 1; arrestIndex < inmate.arrestinfo.length; arrestIndex++) {
                    ...
                    var detailBlock = '<table class="table table-hover agencyTable"><tr><th colspan="2" class="text-center">Agency: ' + agency + '</th><td></tr><tr><th>Arresting officer</th><td>' + officer + '</td></tr><tr><th>Charge/bond details</th><td>';
                    ...
                    if (inmate.arrestinfo[arrestIndex].of.length > 0) {
                        ...
                        for (var offIndex = 0; offIndex < offenses.length; offIndex++) {
                            ...
                            detailBlock += '<div class="offense"><span class="leadin">Offense:</span> ' + thisOff + '<br/>';

                            // if statute is not the same as the offense, create a statute row
                            if (offenses[offIndex].os[0] !== offenses[offIndex].ol[0]) {
                                detailBlock += '<span class="leadin">Statute:</span> ' + offenses[offIndex].os[0] + '<br/>';
                            };
                            detailBlock += '<span class="leadin">Warrant:</span> ' + offenses[offIndex].ow[0] + '<br/><span class="leadin">Court:</span> ' + offenses[offIndex].oc[0] + '<br/>';

                            // if bond info exists, add it
                            if (offenses[offIndex].ob[0] !== "") {
                                detailBlock += '<span class="leadin">Bond:</span> $' + offenses[offIndex].ob[0] + '</div>';
                            }
                            else {
    ...
    $(filterSource).each( function(index,element){
        ...
        for (var v = 0;v<values.length;v++){
            ...
            if (element.toLowerCase().indexOf(values[v]) === -1) {
                ...
                if (values[v] === "male" && element.toLowerCase().indexOf("female") > -1) {
                    isMatched = false;
                    break;
                }
                else
